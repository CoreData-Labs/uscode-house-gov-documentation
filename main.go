...
import (
	...
	"regexp"        // For extracting links from raw HTML using regex
...
// downloadLatestRelease fetches the "latest" US Code page, finds the newest
// ZIP file link, and processes it.
func downloadLatestRelease(alreadyDownloaded map[string]bool) error {
	log.Println("STEP 1: Finding latest US Code release...")
	latestPageURL := usCodeDownloadRootURL + "download.shtml" // URL of the page listing the newest release
	pageHTML, err := fetchPageHTML(latestPageURL)             // Fetch the page HTML
	if err != nil {
		return fmt.Errorf("could not fetch latest release page: %w", err)
	}
	// The latest page has a link like: href="releasepoints/.../htm_uscAll@xxx.zip"
	zipPattern := regexp.MustCompile(`href="(releasepoints/[^"]*htm_uscAll@[^"]+\.zip)"`)
	match := zipPattern.FindStringSubmatch(pageHTML) // Find the first matching ZIP link
	if len(match) < 2 {
		return fmt.Errorf("could not find a ZIP download link on the latest release page")
	}
	// Resolve the relative ZIP path against the base download URL to get the full URL
	fullZipURL, err := resolveURL(usCodeDownloadRootURL, match[1])
	if err != nil {
		return fmt.Errorf("could not resolve ZIP URL: %w", err)
	}
...
// extractReleasePageLinks scans the prior-releases index page and returns a
// deduplicated list of relative URLs pointing to individual release pages.
func extractReleasePageLinks(indexPageHTML string) ([]string, error) {
	// Each release has a link like: href="releasepoints/xxx.htm"
	linkPattern := regexp.MustCompile(`href="(releasepoints/[^"]+\.htm)"`)
	matches := linkPattern.FindAllStringSubmatch(indexPageHTML, -1) // Find all matching links
	seen := make(map[string]bool)                                   // Track links we've already added to avoid duplicates
	var uniqueLinks []string                                        // The final deduplicated list
	for _, match := range matches {
		if len(match) > 1 && !seen[match[1]] { // Only add if we haven't seen this link before
			seen[match[1]] = true
			uniqueLinks = append(uniqueLinks, match[1])
		}
	}
	if len(uniqueLinks) == 0 {
		return nil, fmt.Errorf("no release page links found on the prior releases index")
	}
	return uniqueLinks, nil
}

// extractZipLinkFromReleasePage scans an individual release page and returns
// the relative path of the ZIP file listed on it.
func extractZipLinkFromReleasePage(releasePageHTML string) (string, error) {
	// ZIP links on release pages look like: href="htm_uscAll@xxx.zip"
	zipPattern := regexp.MustCompile(`href="(htm_uscAll@[^"]+\.zip)"`)
	match := zipPattern.FindStringSubmatch(releasePageHTML) // Find the first ZIP link
	if len(match) < 2 {
		return "", fmt.Errorf("no ZIP link found on this release page")
	}
	return match[1], nil
}
...
func downloadAllHistoricalReleases(alreadyDownloaded map[string]bool) error {
	...
	releasePageRelativeLinks, err := extractReleasePageLinks(indexHTML) // Get all release page links
	if err != nil {
		return err
	}
	...
	for _, relativeLink := range releasePageRelativeLinks { // Visit each release page one by one
		...
		zipRelativePath, err := extractZipLinkFromReleasePage(releasePageHTML) // Find the ZIP on this page
		if err != nil {
